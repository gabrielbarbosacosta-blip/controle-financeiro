(function(){
  ...
  function selectedMonth(){
    try{const v=String(state?.settings?.selectedMonth||window.state?.settings?.selectedMonth||'').slice(0,7);if(/^\d{4}-\d{2}$/.test(v))return v}catch(e){}
    for(const selector of ['#monthSelect','#selectedMonth','#projectionMonth','#historyMonth','input[type="month"]']){const v=String(document.querySelector(selector)?.value||'');if(/^\d{4}-\d{2}$/.test(v))return v}
    const d=new Date();return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`;
  }
  function monthLabel(ym){try{return typeof fmtMonth==='function'?fmtMonth(ym):ym}catch(e){return ym}}
  function monthOf(e){return String(e?.date||'').slice(0,7)}
  function isPending(e){const s=String(e?.status||'').trim().toLowerCase();return !['pago','recebido'].includes(s)}
  function cents(v){return Math.round((Number(v)||0)*100)}
  function monthReceive(b,month){return (Array.isArray(b?.entries)?b.entries:[]).filter(e=>e?.direction==='receive'&&monthOf(e)===month&&isPending(e)).reduce((s,e)=>s+(Number(e.amount)||0),0)}
  function chargeKey(b,month,amount){const who=String(b?.userId||b?.participantUserId||b?.name||'unknown').replace(/[^a-zA-Z0-9_-]/g,'_').slice(0,120);return `shared-charge:${month}:${who}:${cents(amount)}`}

  function injectStyles(){
    if(document.getElementById(STYLE_ID))return;
    const style=document.createElement('style');style.id=STYLE_ID;style.textContent=`
      #profileNotificationBtn{position:relative}#profileNotificationBtn .notification-badge{position:absolute;right:1px;top:0;min-width:15px;height:15px;padding:0 4px;border-radius:999px;background:#ef4444;color:#fff;font-size:9px;font-weight:800;line-height:15px;text-align:center;border:2px solid #172033;display:none}#profileNotificationBtn.has-notifications .notification-badge{display:block}
      .notifications-backdrop{position:fixed;inset:0;z-index:1100;background:rgba(2,6,23,.08);backdrop-filter:blur(7px);-webkit-backdrop-filter:blur(7px);opacity:0;visibility:hidden;pointer-events:none;transition:opacity .2s ease,visibility 0s linear .22s}.notifications-backdrop.open{opacity:1;visibility:visible;pointer-events:auto;transition:opacity .2s ease,visibility 0s linear 0s}
      .notifications-panel{position:fixed;z-index:1200;width:min(440px,calc(100vw - 34px));max-height:min(76vh,680px);overflow:auto;padding:12px;background:#111827;border:1px solid #334155;border-radius:15px;box-shadow:0 24px 64px rgba(0,0,0,.52);opacity:0;visibility:hidden;pointer-events:none;transform:translateY(-9px) scale(.985);transform-origin:top left;transition:opacity .2s ease,transform .24s cubic-bezier(.2,.8,.2,1),visibility 0s linear .24s}.notifications-panel.open{opacity:1;visibility:visible;pointer-events:auto;transform:translateY(0) scale(1);transition:opacity .2s ease,transform .24s cubic-bezier(.2,.8,.2,1),visibility 0s linear 0s}
      .notifications-head{display:flex;align-items:center;justify-content:space-between;gap:12px;margin:1px 2px 10px}.notifications-title{font-size:14px;font-weight:800;color:#f8fafc}.notifications-count{font-size:10px;color:#94a3b8}.notifications-list{display:grid;gap:9px}.notification-card{padding:13px 14px;border:1px solid var(--line);border-radius:13px;background:#0f172a}.notification-type{font-size:10px;font-weight:800;letter-spacing:.03em;text-transform:uppercase;color:#93c5fd;margin-bottom:5px}.notification-card.charge .notification-type{color:#fde68a}.notification-title{font-size:13px;font-weight:780;color:#f8fafc}.notification-meta{font-size:11px;color:var(--muted);line-height:1.5;margin-top:5px}.notification-value{font-size:16px;font-weight:800;margin-top:8px}.notification-actions{display:flex;gap:7px;flex-wrap:wrap;margin-top:11px}.notification-empty{padding:28px 14px;text-align:center;color:var(--muted);font-size:12px}
      .finance-toast-host{position:fixed;right:22px;bottom:22px;z-index:1400;width:min(370px,calc(100vw - 30px));pointer-events:none}.finance-toast{pointer-events:auto;display:grid;grid-template-columns:38px minmax(0,1fr) 24px;gap:10px;align-items:start;padding:13px 13px 13px 12px;background:#111827;border:1px solid #334155;border-radius:14px;box-shadow:0 20px 55px rgba(0,0,0,.48);opacity:0;transform:translateY(18px) scale(.98);transition:opacity .22s ease,transform .28s cubic-bezier(.2,.8,.2,1);cursor:pointer}.finance-toast.show{opacity:1;transform:translateY(0) scale(1)}.finance-toast.hide{opacity:0;transform:translateY(10px) scale(.985)}.finance-toast-icon{width:38px;height:38px;border-radius:11px;display:grid;place-items:center;background:#102845;border:1px solid #1d497b;color:#93c5fd}.finance-toast-icon svg{width:19px;height:19px;fill:none;stroke:currentColor;stroke-width:1.9;stroke-linecap:round;stroke-linejoin:round}.finance-toast-title{font-size:12px;font-weight:800;color:#f8fafc;line-height:1.35}.finance-toast-text{font-size:11px;color:#cbd5e1;line-height:1.45;margin-top:3px}.finance-toast-hint{font-size:10px;color:#60a5fa;margin-top:6px;font-weight:700}.finance-toast-close{width:24px;height:24px;border:0;background:transparent;color:#94a3b8;padding:0;border-radius:7px;font-size:17px;line-height:1;display:grid;place-items:center}
      @media(max-width:900px){.notifications-panel{left:15px!important;right:15px!important;top:90px!important;width:auto;max-height:calc(100vh - 109px);transform-origin:top center}.finance-toast-host{left:15px;right:15px;bottom:15px;width:auto}}@media(prefers-reduced-motion:reduce){.notifications-panel,.notifications-backdrop,.finance-toast{transition:none!important}}
  ...
  function bindBell(){const b=bell();if(!b)return false;if(!b.querySelector('.notification-badge')){const badge=document.createElement('span');badge.className='notification-badge';badge.setAttribute('aria-hidden','true');b.appendChild(badge)}if(b.dataset.notificationsBound!=='1'){b.dataset.notificationsBound='1';b.title='Notificações';b.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();togglePanel()});b.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();e.stopPropagation();togglePanel()}else if(e.key==='Escape')closePanel()})}return true}
  function positionPanel(){const card=document.getElementById('profileSidebarCard');if(!card||!panel||window.innerWidth<=900)return;const r=card.getBoundingClientRect(),width=Math.min(440,window.innerWidth-34),maxLeft=Math.max(16,window.innerWidth-width-16);panel.style.left=Math.min(Math.max(16,r.left),maxLeft)+'px';panel.style.top=(r.bottom+7)+'px'}
  async function openPanel(sharedId){ensureUi();document.querySelector('body > .profile-dropdown.open')?.classList.remove('open');document.body.classList.remove('profile-menu-open');positionPanel();await loadNotifications();backdrop.classList.add('open');requestAnimationFrame(()=>{panel.classList.add('open');window.financeNotificationQueueRefresh?.();if(sharedId)setTimeout(()=>panel.querySelector(`[data-notification-shared="${CSS.escape(String(sharedId))}"]`)?.scrollIntoView({block:'nearest',behavior:'smooth'}),80)})}
  ...
  function detectNewInvitations(nextItems){const seen=readSeen();let changed=false;nextItems.filter(i=>i.myStatus==='pending').forEach(i=>{const id=String(i.id||'');if(!id||seen.has(id))return;seen.add(id);changed=true;queueToast(i)});if(changed)writeSeen(seen)}
  function queueToast(item){toastQueue.push(item);showNextToast()}
  function showNextToast(){if(toastActive||!toastQueue.length)return;ensureUi();toastActive=true;const item=toastQueue.shift(),creator=creatorLabel(item),toast=document.createElement('div');toast.className='finance-toast';toast.setAttribute('role','status');toast.innerHTML=`<div class="finance-toast-icon"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 7h8M8 11h6M7 3h10a2 2 0 0 1 2 2v14l-4-3H7a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2Z"/></svg></div><div><div class="finance-toast-title">${esc(creator)} cadastrou uma despesa</div><div class="finance-toast-text">${esc(item.description||'Nova despesa compartilhada')} · ${money(item.isPayer?Number(item.amount)||0:Number(item.myAmount)||0)}</div><div class="finance-toast-hint">Clique para ver</div></div><button type="button" class="finance-toast-close" aria-label="Fechar">×</button>`;toastHost.appendChild(toast);let closed=false,timer;const finish=()=>{if(closed)return;closed=true;clearTimeout(timer);toast.classList.add('hide');setTimeout(()=>{toast.remove();toastActive=false;showNextToast()},230)};toast.querySelector('.finance-toast-close').onclick=e=>{e.stopPropagation();finish()};toast.onclick=()=>{finish();openPanel(item.id)};requestAnimationFrame(()=>toast.classList.add('show'));timer=setTimeout(finish,7000)}
  ...
  async function markVisibleChargesViewed(){const host=document.getElementById('notificationsList');if(!host)return;const keys=[...new Set([...host.querySelectorAll('.notification-card.charge[data-notification-key]')].filter(c=>c.dataset.queueActive==='1'&&c.style.display!=='none').map(c=>c.dataset.notificationKey).filter(Boolean))];for(const key of keys)await markChargeViewed(key)}
