...
function renderHistory(){
 const q=document.getElementById('searchFilter').value.toLowerCase().trim(),type=document.getElementById('typeFilter').value,cat=document.getElementById('categoryFilter').value,status=document.getElementById('statusFilter').value;
 const rows=historyRowsForMonth(state.settings.selectedMonth).filter(r=>!type||r.type===type).filter(r=>!cat||r.category===cat).filter(r=>!status||r.status===status).filter(r=>!q||`${r.description} ${r.account} ${r.notes}`.toLowerCase().includes(q)).sort((a,b)=>String(b.date).localeCompare(String(a.date)));
 document.getElementById('historyCount').textContent=`${rows.length} item(ns) em ${fmtMonth(state.settings.selectedMonth)}`;document.getElementById('historyBody').innerHTML=rows.length?rows.map(r=>`<tr><td>${r.kind==='invoice'?fmtMonth(state.settings.selectedMonth):fmtDate(r.date)}</td><td><span class="badge ${r.type.toLowerCase().replace('í','i')}">${r.type}</span></td><td>${r.description}</td><td>${r.category||'—'}</td><td>${r.account||'—'}</td><td><span class="badge ${String(r.status).toLowerCase()}">${r.status}</span></td><td class="num ${r.type==='Receita'?'positive':r.type==='Benefício'?'':'negative'}">${fmtMoney(r.amount)}</td><td>${r.kind==='invoice'?`<button class="btn small" onclick="openInvoiceFromHistory('${r.id}')">Abrir</button>`:`<button class="btn small" onclick="editTx('${r.id}')">Editar</button>`}</td></tr>`).join(''):'<tr><td colspan="8" class="empty">Nenhum item encontrado.</td></tr>'
}
