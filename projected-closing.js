(function(){
  function fallbackProjectedClosing(ym){
    ...
    const pending=type=>state.transactions.filter(t=>String(t.type||'').toLowerCase()===type&&String(t.status||'').toLowerCase()==='pendente'&&String(t.date||'').slice(0,7)===ym).reduce((s,t)=>s+(Number(t.amount)||0),0);
    let expense=pending('despesa');
    if(Array.isArray(state.cards)&&typeof cardForecast==='function')expense+=state.cards.filter(c=>c.active!==false).filter(c=>typeof getInvoice!=='function'||String(getInvoice(c.id,ym)?.status||'').toLowerCase()!=='paga').reduce((s,c)=>s+(Number(cardForecast(c,ym).total)||0),0);
    return (typeof round2==='function'?round2:v=>Math.round((Number(v)||0)*100)/100)(actual+pending('receita')-expense);
  }
  ...
  function init(){renderProjectedClosing();const dashboard=document.getElementById('page-dashboard');if(dashboard)observer.observe(dashboard,{childList:true,subtree:true,characterData:true});const month=document.getElementById('monthSelect');if(month)month.addEventListener('change',()=>setTimeout(renderProjectedClosing,0))}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',init);else init();
})();

(function(){
  if(document.querySelector('script[src="income-simulator-comparison.js"]'))return;
  const script=document.createElement('script');script.src='income-simulator-comparison.js';script.async=false;document.body.appendChild(script);
})();
