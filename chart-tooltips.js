(function(){
  ...
  function tooltipFor(canvas){
    const host=canvas?.parentElement;if(!host)return null;host.style.position='relative';
    let tip=host.querySelector(':scope > .financial-chart-tooltip');
    if(!tip){tip=document.createElement('div');tip.className='financial-chart-tooltip';tip.style.cssText='display:none;position:absolute;z-index:30;pointer-events:none;min-width:120px;padding:7px 9px;border:1px solid #334155;border-radius:8px;background:#0f172a;color:#e2e8f0;font-size:12px;line-height:1.35;box-shadow:0 8px 24px rgba(0,0,0,.28);white-space:nowrap;transform:translate(-50%,-100%)';host.appendChild(tip)}
    return tip;
  }
  function bind(canvas,points){
    if(!canvas||!Array.isArray(points)||!points.length)return;canvas.__financialTooltipPoints=points;
    const tip=tooltipFor(canvas);if(!tip||canvas.__financialTooltipBound)return;canvas.__financialTooltipBound=true;
    canvas.addEventListener('mousemove',e=>{
      const list=canvas.__financialTooltipPoints||[],rect=canvas.getBoundingClientRect(),mx=e.clientX-rect.left,my=e.clientY-rect.top;let nearest=null,best=Infinity;
  ...
  function alignEventMarkers(canvas,data){
    ...
    const{x,y}=geometry(canvas,data);[...layer.children].forEach(marker=>{const left=parseFloat(marker.style.left);if(!Number.isFinite(left))return;let idx=0,best=Infinity;data.forEach((_,i)=>{const d=Math.abs(x(i)-left);if(d<best){best=d;idx=i}});marker.style.top=`${y(data[idx].value)-16}px`});
  }
  ...
  function install(){
    const current=window.drawLineChart,canonical=window.__cadernoDarkChartRenderer;
    const base=typeof canonical==='function'?canonical:(typeof current==='function'?(current.__cadernoBaseRenderer||current):null);if(typeof base!=='function')return false;
    if(current?.__cadernoTooltipOnly&&current.__cadernoBaseRenderer===base)return true;
    const wrapped=function(id,data){if(id==='projectionChart'&&window.__financeChartLineIntroActive===true)return;const result=base.apply(this,arguments);attach(id,data);return result};
    wrapped.__cadernoTooltipOnly=true;wrapped.__cadernoBaseRenderer=base;window.drawLineChart=wrapped;try{drawLineChart=wrapped}catch(e){};return true;
  }
  ...
  window.financeRefreshChartTooltips=function(){
    try{
      install();const ym=state?.settings?.selectedMonth;if(!ym)return;let rows=[];
      ...
      const data=(rows||[]).map(r=>({label:typeof fmtMonth==='function'?fmtMonth(r.ym):r.ym,value:r.closing}));attach('projectionChart',data);
    }catch(e){}
  };
})();
