(function(){
  ...
  function routeFrom(trigger){
    ...
    const hist=trigger.closest('#historyBody tr');if(hist){const id=parseInline(hist.querySelector('button[onclick*="editTx("]'),'editTx');if(id)return{kind:'transaction',id}}
  ...
  function render(e){
    const panel=ensureShell(),rows=occurrences(e),next=nextOccurrence(e),done=realizedTotal(e),remaining=remainingTotal(e),future=impact12(e),system=systemImpact12(e),share=system>0?Math.min(100,future/system*100):0,status=entityStatus(e),amount=currentAmount(e),expense=isExpenseEntity(e),hist=history(e),details=info(e),series=chartSeries(e),canEdit=e.kind!=='transaction'||!String(e.id).startsWith('shared-');
    const kpis=expense
    ...
    panel.innerHTML=`<div class="pfp-head"><div class="pfp-head-main"><div class="pfp-eyebrow">Painel ${expense?'da despesa':'da receita'} · versão de teste</div><div class="pfp-title">${esc(entityName(e))}</div><div class="pfp-meta"><span class="pfp-chip ${expense?'warn':'ok'}">${esc(entityType(e))}</span><span class="pfp-chip">${esc(e.item.category||'Sem categoria')}</span><span class="pfp-chip ${status==='Ativa'?'ok':''}">${esc(status)}</span>${sharedReceivable(e.item)?'<span class="pfp-chip info">Reembolso</span>':''}</div></div><div class="pfp-actions">${canEdit?'<button type="button" class="btn" data-pfp-edit>Editar</button>':''}<button type="button" class="pfp-close" data-pfp-close aria-label="Fechar painel">×</button></div></div>
  ...
  function init(){
    ...
    document.addEventListener('click',ev=>{
      const trigger=ev.target?.closest?.('.pfp-panel-btn,[data-pfp-link]');if(!trigger)return;const route=routeFrom(trigger);if(!route)return;const entity=resolve(route.kind,route.id);if(!entity)return;ev.preventDefault();ev.stopImmediatePropagation();open(entity);
    },true);
    document.addEventListener('keydown',ev=>{if(ev.key==='Escape'&&document.body.classList.contains('prumo-financial-panel-open')&&!document.querySelector('.modal-backdrop.open')){ev.preventDefault();close()}});
