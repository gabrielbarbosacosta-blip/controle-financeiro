(function(){
    ...
    if(main&&!document.getElementById('page-debts')){
      ...
      page.innerHTML=`
        <div class="section-head">
          <div><h3>Dívidas parceladas</h3><div class="muted">Cadastre uma dívida uma vez. Cada parcela será criada automaticamente no mês correspondente.</div></div>
        ...
        <div class="notice" style="margin-bottom:14px">As parcelas aparecem em <strong>Lançamentos</strong> como despesas pendentes. O status pode ser alterado diretamente para <strong>Pago</strong> ou <strong>Pendente</strong>.</div>
        <div class="card">
    ...
    if(!document.getElementById('debtModal')){
      ...
      wrap.innerHTML=`
        <div class="modal-backdrop" id="debtModal"><div class="modal"><form id="debtForm">
          ...
          <div class="modal-body"><div class="notice" style="margin-bottom:14px">Informe a parcela que começa no mês escolhido. O sistema criará automaticamente uma despesa para cada mês restante.</div><div class="form-grid">
            ...
            <div class="field"><label>Total de parcelas</label><input id="debtTotalInstallments" type="number" min="1" step="1" required></div>
  ...
  function syncDebtTransactions(debt){
    ...
    const generated=[];
    const first=Math.max(1,Number(debt.firstInstallment)||1),total=Math.max(first,Number(debt.totalInstallments)||first);
    for(let n=first;n<=total;n++){
      const ym=addMonth(debt.firstMonth,n-first),previous=byNumber.get(n),status=previous?.status||'Pendente';
      generated.push({
        id:previous?.id||txUid(),
        date:safeDate(ym,debt.dueDay),
        type:'Despesa',
        category:debt.category||'Dívidas',
        description:`${debt.name} — parcela ${n}/${total}`,
        account:debt.account||'',
        nature:'Parcelamento',
        amount:Number(debt.installmentAmount)||0,
        status,
        notes:[`Parcela gerada automaticamente pela dívida "${debt.name}".`,debt.notes||''].filter(Boolean).join(' '),
        projection:true,
        recurring:false,
  ...
  function openDebtModal(id=null){
    ...
    document.getElementById('debtTotalInstallments').value=debt?.totalInstallments??'';
  ...
  function saveDebtFromForm(e){
    ...
    const total=Math.max(1,Number(document.getElementById('debtTotalInstallments').value)||1);
    const first=Math.max(1,Number(document.getElementById('debtFirstInstallment').value)||1);
    if(first>total){alert('A parcela inicial não pode ser maior que o total de parcelas.');return}
    const debt={
      id,
  ...
  function renderDebtPage(){
    ...
    body.innerHTML=debts.length?debts.map(d=>{
      const txs=linkedTransactions(d.id).sort((a,b)=>String(a.date).localeCompare(String(b.date))),pend=txs.filter(t=>String(t.status).toLowerCase()==='pendente'),paid=txs.length-pend.length,next=pend[0],remaining=pend.reduce((s,t)=>s+(Number(t.amount)||0),0),pct=txs.length?Math.round(paid/txs.length*100):0;
      return `<tr>
        <td><div class="debt-name">${esc(d.name)}</div><div class="debt-sub">${esc(d.account||'Conta não informada')} • ${esc(d.category||'Dívidas')}</div><div class="debt-progress"><span style="width:${pct}%"></span></div></td>
        <td>${paid} paga(s) • ${pend.length} pendente(s)<div class="debt-sub">${d.firstInstallment}/${d.totalInstallments} até ${d.totalInstallments}/${d.totalInstallments}</div></td>
        <td>${monthLabel(d.firstMonth)}</td>
        <td>${next?`${monthLabel(String(next.date).slice(0,7))}<div class="debt-sub">parcela ${next.debtInstallmentNumber}/${d.totalInstallments}</div>`:'Quitada'}</td>
