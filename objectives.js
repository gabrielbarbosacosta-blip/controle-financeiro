(function(){
  ...
  const TYPES={
    house:{label:'Casa própria',icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m3 10 9-7 9 7v11h-7v-7h-4v7H3z"/></svg>'},
    motorcycle:{label:'Moto',icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="5" cy="17" r="4"/><circle cx="19" cy="17" r="4"/><path d="m5 17 5-8 5 8H5m9-13h3l2 13M8 9h7"/></svg>'},
    car:{label:'Carro',icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m3 10 2-6h14l2 6v10h-3v-3H6v3H3zm0 0h18M6 13h2m8 0h2"/></svg>'},
    reserve:{label:'Reserva',icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="4"/><path d="m5.6 5.6 3.6 3.6m5.6 5.6 3.6 3.6m0-12.8-3.6 3.6m-5.6 5.6-3.6 3.6"/></svg>'},
    travel:{label:'Viagem',icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m21 3-6 18-4-8-8-4zM11 13 21 3"/></svg>'},
    education:{label:'Estudos',icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m2 9 10-5 10 5-10 5zM6 11v6q6 5 12 0v-6m4-2v8"/></svg>'},
    other:{label:'Outro objetivo',icon:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="5"/><circle cx="12" cy="12" r="1"/></svg>'}
  };
  ...
  function injectStyles(){
    ...
    const s=document.createElement('style');s.id=STYLE_ID;s.textContent=`
      ...
      .goal-icon svg{width:24px;height:24px;display:block}
  ...
  function openContributionHistory(id){
    ...
    document.getElementById('goalContributionHistoryTitle').textContent=`Movimentações · ${g.name}`;
    document.getElementById('goalContributionHistorySummary').textContent=`${list.length} movimentação${list.length===1?'':'ões'} registrada${list.length===1?'':'s'}`;
    const host=document.getElementById('goalContributionHistoryList');
    host.innerHTML=list.length?list.map(c=>contributionRowHtml(c,true)).join(''):'<div class="goal-contrib-empty">Nenhuma movimentação registrada ainda.</div>';
